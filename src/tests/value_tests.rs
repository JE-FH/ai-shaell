...
fn test_value_number_to_sstring() {
    ...
    assert_eq!(Value::Number(2.5).to_sstring().unwrap(), "2.5");
}

#[test]
fn test_value_string_to_bool() {
    assert!(!Value::String("".to_string()).to_bool().unwrap());
    assert!(Value::String("x".to_string()).to_bool().unwrap());
}
...
fn test_value_bstring_to_bool() {
    assert!(!Value::BString(vec![]).to_bool().unwrap());
    assert!(Value::BString(vec![1, 2, 3]).to_bool().unwrap());
}
...
fn test_index_out_of_bounds_returns_null() {
    ...
    let result = heap.with_ref(t, |v| match v {
        Value::Table(td) => {
            if !td.array.is_empty() {
                td.array[0]
            } else {
                GcRef::NULL
            }
